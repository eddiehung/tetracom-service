class UsersController < ApplicationController
	...
	def index
		if current_user.admin?
			@users = params[:search] ? Kaminari.paginate_array(User.search_admin(params[:search])).page(params[:page]).per(10) : User.order(:name).page(params[:page]).per(10)
		else
			@users = params[:search] ? Kaminari.paginate_array(User.search(params[:search])).page(params[:page]).per(10) : User.order(:name).page(params[:page]).per(10)
		end
	end
